...
import json
from mytorch.nn import Identity, Sigmoid, Tanh, ReLU, GELU, Swish, Softmax
...
DEBUG_AND_GRADE_Swish = DEBUG_AND_GRADE_Swish_flag

if DEBUG_AND_GRADE_Swish:

    print("\n──────────────────────────────────────────")
    print("Swish | STUDENT OUTPUT")
    print("──────────────────────────────────────────")
    ...
    print("\n──────────────────────────────────────────")
    print("Swish | SOLUTION OUTPUT")
    print("──────────────────────────────────────────")
   
    A_solution =np.array([
        [-0.07194484, -0.14227763],
        [-0.23840584, -0.26894143],
        [ 0.,          0.7310586 ],
        [ 1.761594,    2.8577223 ],
    ])

    dLdZ_solution = np.array([
        [-0.05266462, -0.08810411],
        [-0.27235273,  0.07232949],
        [ 1.,          0.        ],
        [ 0.,         -1.0881041 ],
    ])

    dLdbeta_solution = np.array([1.7391382])
    ...
    print("\n──────────────────────────────────────────")
    print("Swish | CLOSENESS TEST RESULT")
    print("──────────────────────────────────────────")

    print("\n           Pass?")

    TEST_swish_A = np.allclose(A.round(4), A_solution, atol=atol_threshold)
    print("Test A:   ", TEST_swish_A)

    TEST_swish_dLdbeta = np.allclose(dLdbeta.round(4), dLdbeta_solution, atol=atol_threshold)
    print("Test dLdbeta:", TEST_swish_dLdbeta)

    TEST_swish_dLdZ = np.allclose(dLdZ.round(4), dLdZ_solution, atol=atol_threshold)
    print("Test dLdZ:", TEST_swish_dLdZ)

else:

    TEST_swish_A = False
    TEST_swish_dLdbeta = False
    TEST_swish_dLdZ = False
